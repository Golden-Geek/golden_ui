<script lang="ts">
    ...
    let isSelected = $derived(
        session?.isNodeSelected(node?.node_id ?? -1) ?? false,
    );
</script>

{#if node}
    <div class="outliner-item">
        <div class="outliner-item-content" class:selected={isSelected}>
        ...
        {#if isExpanded && node.children && node.children.length > 0 && (maxLevels == undefined || level < maxLevels)}
            <div
...
<style>
    .outliner-item {
        ...
        font-size: 0.8rem;
        width: 100%;
        /* border-left: 1px solid var(--gc-color-outliner-border); */
    }
